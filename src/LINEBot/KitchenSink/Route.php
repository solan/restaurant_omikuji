            ...
            foreach ($events as $event) {
                ...
                if ($event instanceof MessageEvent) {
                    if ($event instanceof TextMessage) {
                       $handler = new TextMessageHandler($bot, $logger, $req,$event,$hotpepper,$mongoDBURI);
                    } elseif ($event instanceof StickerMessage) {
                       $bot->replyText($event->getReplyToken(), "位置情報のみを送ってください");
                    } elseif ($event instanceof LocationMessage) {
                       $handler = new LocationMessageHandler($bot, $logger, $event,$hotpepper,$mongoDBURI);
                ...
                elseif ($event instanceof PostbackEvent) {
                    $handler = new PostbackEventHandler($bot, $logger, $event,$mongoDBURI);
                } 
                else {
                    // Unexpected behavior (just in case)
                    // something wrong if reach here
